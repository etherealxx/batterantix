    ...
    if triggerOnce == 0:
        if int(powerStat)<=int(lowBatteryPercentage) and chargeStat == "Discharging":
            if int(powerStat)<=int(criticalBatteryPercentage):
                os.system('play -q -v 0.3 ' + curdir + '/win10-lowbatt.mp3 & zenity --warning --text="Sistem akan segera dimatikan" --timeout=5')
                #os.system('poweroff')
                triggerOnce = 1
            else:
                os.system('play -q -v 0.3 ' + curdir + '/win10-lowbatt.mp3 & zenity --warning --text="Baterai tersisa ' + powerStat + '%" --timeout=5')
                triggerOnce = 1
        elif int(powerStat)>=99 and chargeStat == "Full":
            os.system('play -q -v 0.3 ' + curdir + '/win10-lowbatt.mp3 & zenity --warning --text="Baterai penuh" --timeout=5')
            triggerOnce = 1
